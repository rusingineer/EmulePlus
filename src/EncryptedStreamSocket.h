...
public:
	...
	void	SetConnectionEncryption(bool bEnabled, const uchar *pTargetClientHash);
...
#else
	bool	IsObfusicating() const				{ return m_StreamCryptState == ECS_ENCRYPTING; }
#endif
	
	bool	IsServerCryptEnabledConnection() const { return m_bServerCrypt; }	

	byte	m_dbgbyEncryptionSupported;
	byte	m_dbgbyEncryptionRequested;
	byte	m_dbgbyEncryptionMethodSet;
...
protected:
	...
	byte			GetSemiRandomNotProtocolMarker() const;
